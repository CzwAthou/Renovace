...
public class RenovaceFunc<T> implements Function<ResponseBody, T> {
    ...
    private boolean inteceptorResp(int code, String extra) {
        if (RequestFactory.getRespCodeInteceptorList() != null && !RequestFactory.getRespCodeInteceptorList().isEmpty()) {
            for (RespCodeInteceptor inteceptor : RequestFactory.getRespCodeInteceptorList()) {
